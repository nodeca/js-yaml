...
module.exports = function parse(input, settings) {
  ...
  coreSchema.implicit.forEach(function (type) {
    implicitInterpreters.push(type.interpreter);
    explicitInterpreters[type.tag] = type.interpreter;
  });
  ...
  function composeNode(nodeIndent, withinFlowNode) {
    ...
    if ('!' !== tag) {
      if ('?' === tag) {
        for (index = 0, amount = implicitInterpreters.length;
             index < amount;
             index += 1) {
          _result = implicitInterpreters[index](result, false);
      ...
      } else if (_hasOwn.call(explicitInterpreters, tag)) {
        _result = explicitInterpreters[tag](result, true);
