...
module.exports = function parse(input, settings) {
  ...
  function throwParseError(message) {
    throw new Error(
      util.format(
        'YAML parse error: %s at position %d (line %d, column %d)',
  ...
  // TODO: Join with `skipFlowSeparationSpace`
  function skipBlockSeparationSpace() {
    var _position = position, atNewLine = false;
  ...
  function readPlainScalar(indent, withinFlowNode) {
    ...
    while (position < length) {
      if (CHAR_COLON === character) {
        ...
        if (performFlowFolding(indent, false)) {
  ...
  function readSingleQuotedScalar(indent) {
    ...
    while (position < length) {
      if (CHAR_SINGLE_QUOTE === character) {
        ...
        performFlowFolding(indent, true);
        captureStart = position;
        character = input.charCodeAt(position);
  ...
  function readBlockSequence(indent) {
    var following, _indent, _result = [], detected = false;
  ...
  function readImplicitBlockMapping(indent) {
    var following, _indent, _line,
    ...
    while (position < length) {
      ...
      if (!readFlowNode(indent, false) || line !== _line) {
        if (detected) {
          throwParseError('can not read key node of a mapping');
        } else {
          break;
        }
      }
      ...
      if (CHAR_COLON === character) {
        character = input.charCodeAt(++position);
        keyNode = String(result);
        detected = true;

        if (safeMode && _hasOwn.call(_result, keyNode)) {
          throwParseError('duplication of a mapping key');
        }
      } else if (detected) {
        throwParseError('can not read a mapping pair; a colon is missed');
      } else {
        return true; // return the result of readFlowNode
      }

      if (skipBlockSeparationSpace()) {
        _result[keyNode] = null;

        if (lineIndent > indent) {
          _indent = lineIndent;
          readBlockNode(_indent);
        } else if (lineIndent === indent) {
          continue;
        } else {
          break;
        ...
        readFlowNode(indent + 1, false);
        skipBlockSeparationSpace();
      }
  ...
  function readFlowNode(indent, withinFlowNode) {
    return readPlainScalar(indent, withinFlowNode) ||
           readSingleQuotedScalar(indent) ||
           readDoubleQuotedScalar(indent) ||
           readFlowCollection(indent);
  }

  function readBlockNode(indent) {
    return readBlockSequence(indent) ||
           readImplicitBlockMapping(indent);
  }

  function readNode(indent) {
    return readBlockNode(indent) || readFlowNode(indent, false);
  }
