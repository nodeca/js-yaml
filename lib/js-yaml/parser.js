...
module.exports = function parse(input, settings) {
  ...
  function foldLines(expectedIndent) {
    var actualIndent, emptyLines = -1, result = '';
  ...
  function readSingleQuotedScalar(indent) {
    var scalarResult = '', captureStart;

    if (CHAR_SINGLE_QUOTE === character) {
      character = input.charCodeAt(++position);
      captureStart = position;
    } else {
      throwParseError('expected single quoted scalar');
    }

    while (position < length) {
      if (CHAR_SINGLE_QUOTE === character) {
        scalarResult += capture(captureStart, position, true);
        character = input.charCodeAt(++position);
        ...
        scalarResult += capture(captureStart, findContentEnd(captureStart), true);
        scalarResult += foldLines(indent);
        captureStart = position;
        character = input.charCodeAt(position);
  ...
  function readDoubleQuotedScalar(indent) {
    var captureStart,
        hexLength, hexIndex, hexOffset, hexResult,
        scalarResult = '';
    ...
    while (position < length) {
      if (CHAR_DOUBLE_QUOTE === character) {
        ...
        if (CHAR_LINE_FEED === character || CHAR_CARRIAGE_RETURN === character) {
          ...
          scalarResult += SIMPLE_ESCAPE_SEQUENCES[character];
          character = input.charCodeAt(++position);
          ...
          scalarResult += String.fromCharCode(hexResult);
          character = input.charCodeAt(++position);
  ...
  function readNode(indent) {
    if (CHAR_SINGLE_QUOTE === character) {
      return readSingleQuotedScalar(indent);
    } else if (CHAR_DOUBLE_QUOTE === character) {
      return readDoubleQuotedScalar(indent);
    } else {
      throwParseError('can not determine node type');
    }
  }
