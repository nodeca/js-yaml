...
module.exports = function parse(input, settings) {
  ...
  function readSeparation(expectedIndent, ensureIndented, includePrefix) {
    var actualIndent = 0,
        _position = position,
    ...
    if (CHAR_LINE_FEED === character || CHAR_CARRIAGE_RETURN === character) {
      while (position < length) {
        ...
        // Non-empty line.
                   ...
                   CHAR_CARRIAGE_RETURN !== character) {
          if (ensureIndented && expectedIndent < actualIndent) {
            throwParseError(
              'deficient indentation ' +
              '(it should be at least ' + expectedIndent + ' in width' +
              ', but it is ' + actualIndent + ')');
          }
          break;
        }
      }
  ...
  // TODO: Rewrite and join with the flow collection reader.
  function readPairOrNode(indent, isMapping, _result) {
    var explicitPair = false, key = null, value = null, _position, _line = line;
    ...
    if ((explicitPair || line === _line) && CHAR_COLON === character) {
      character = input.charCodeAt(++position);
      _position = position;
      readSeparation(indent, true, true);
  ...
  function readFlowCollection(indent) {
    var _result, isMapping, terminator, readNext = true;
    ...
    while (position < length) {
      ...
      if (character === terminator) {
        ...
        readPairOrNode(indent, isMapping, _result);
        readSeparation(indent, true, true);
        ...
        throwParseError('missed comma between flow collection entries');
      }
    }
