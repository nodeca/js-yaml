...
module.exports = function parse(input, settings) {
  ...
  function foldLines() {
    ...
    while (true) {
      ...
      // If the line is less indented, it should not have any content.
      ...
      // Skip line prefix and/or empty line content.
      while (CHAR_SPACE === character || CHAR_TAB === character) {
        position += 1;
        character = input.charCodeAt(position);
      }

      // If the previous reading loop has stopped at a line break, the line
      // was an empty line. So continue the folding process.
    ...
    while (position < length) {
      if (CHAR_DOUBLE_QUOTE === character) {
        ...
        if (CHAR_LINE_FEED === character || CHAR_CARRIAGE_RETURN === character) {
          ...
          escapeSequenceLength = HEXADECIMAL_ESCAPE_SEQUENCES[character];
          position += 1;
          character = parseInt(input.substr(position, escapeSequenceLength), 16);

          if (!isNaN(character)) {
            object += String.fromCharCode(character);
            position += escapeSequenceLength;
          } else {
            throwParseError('can not parse hexadecimal number');
          }
        ...
        captureStart = position;
                 ...
                 CHAR_CARRIAGE_RETURN === character) {
        capture(captureStart, captureEnd, true);
        readLineBreak();
        foldLines();
        captureStart = position;
        captureEnd = captureStart + 1;

      } else if (CHAR_SPACE !== character && CHAR_TAB !== character) {
