  ...
  function readBlockMapping(nodeIndent) {
    ...
    while (position < length) {
      ...
      if ((CHAR_QUESTION === character ||
        ...
        if (CHAR_QUESTION === character) {
          detected = true;
          completePair = explicitPair ? true : false;
          explicitPair = true;
        } else if (explicitPair) {
          // i.e. CHAR_COLON === character after the explicit key.
          completePair = true;
        } else {
          throwParseError('incomplete explicit mapping pair; missed a key node');
        }
      ...
      if (explicitPair) {
        ...
        if (lineIndent > nodeIndent) {
          throwParseError('bad indentation within an explicit mapping pair');
        }
      } else {
        if (readFlowNode(nodeIndent, false) && line === _line) {
          ...
          if (CHAR_COLON === character) {
            ...
            // Keep the result of `readFlowNode`.
            return true;
          }
        } else if (detected) {
          throwParseError('can not read a block mapping entry');
        } else {
          return false;
        }
      }
      ...
      if (completePair) {
        _result[String(keyNode)] = valueNode;
        completePair = false;
        keyNode = null;
        valueNode = null;
      }
