...
module.exports = function parse(input, settings) {
  ...
  function readImplicitBlockMapping(nodeIndent) {
    var following, _nodeIndent, _line,
    ...
    while (position < length) {
      ...
      if (!readFlowNode(nodeIndent, false) || line !== _line) {
        if (detected) {
      ...
      if (skipSeparationSpace(-1)) {
        ...
        readFlowNode(nodeIndent + 1, false);
  ...
  function readFlowNode(nodeIndent, withinFlowNode) {
    return readPlainScalar(nodeIndent, withinFlowNode) ||
           readSingleQuotedScalar(nodeIndent) ||
           readDoubleQuotedScalar(nodeIndent) ||
           readFlowCollection(nodeIndent);
  }

  function readBlockNode(nodeIndent) {
    return readBlockSequence(nodeIndent) ||
           readImplicitBlockMapping(nodeIndent);
  }

  function readNode(nodeIndent) {
    return readBlockNode(nodeIndent) || readFlowNode(nodeIndent, false);
  }
