...
function testImplicitResolving(state, str) {
  ...
  for (index = 0, length = state.implicitTypes.length; index < length; index += 1) {
    ...
    if (type.loadValidate && type.loadValidate(str)) {
      return true;
    }
  }
...
function detectType(state, object, explicit) {
  ...
  for (index = 0, length = typeList.length; index < length; index += 1) {
    ...
    if ((type.dumpInstanceOf  || type.dumpPredicate) &&
      ...
      if (type.dumpRepresent) {
        style = state.styleMap[type.tag] || type.dumpDefaultStyle;

        if ('[object Function]' === _toString.call(type.dumpRepresent)) {
          _result = type.dumpRepresent(object, style);
        } else if (_hasOwnProperty.call(type.dumpRepresent, style)) {
          _result = type.dumpRepresent[style](object, style);
        } else {
