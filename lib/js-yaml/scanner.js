...
exports.Scanner = new JS.Class('Scanner', {
  extend: {
    __init__: function (self) {
      ...
      self.tokens = new JS.LinkedList.Doubly.Circular();

      // Add the STREAM-START token.
      self.fetchStreamStart();
      ...
      // In the block context, self flag also signifies if a block collection
      // may start at the current position.
      self.allowSimpleKey = true;
      ...
      // '[', or '{' tokens.
      self.possibleSimpleKeys = new JS.Hash();
    }
  },

  initialize: function () {
    exports.Scanner.__init__(this);
  },
  ...
  checkToken: function () {
    ...
    if (this.tokens.count()) {
      ...
      for (i = 0; i < arguments.length; i++) {
        if (this.tokens.first.isA(arguments[i])) {
          return true;
        }
      }
    }
  ...
  getToken: function () {
    ...
    if (debug.ENABLED) {
      debug('scanner#getToken -- getting token', {tokens: this.tokens});
    }
  ...
  savePossibleSimpleKey: function () {
    ...
    if (this.allowSimpleKey) {
      ...
      tokenNumber = this.tokensTaken + this.tokens.count();
  ...
  scanBlockScalar: function (style) {
    var folded, chunks, startMark, endMark, chomping, increment = null,
        minIndent, indent, breaks, lineBreak, leadingNonSpace, tuple;
    // See the specification for details.
