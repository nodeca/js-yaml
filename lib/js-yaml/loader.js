...
function loadAll(input, output, settings) {
  var name       = common.getOption(settings, 'name',     null),
      ...
      safeMode   = common.getOption(settings, 'validate', true),
      strict     = common.getOption(settings, 'strict',   false),
      legacyMode = common.getOption(settings, 'legacy',   false),
  ...
  function storeMappingPair(mapping, key, value) {
    ...
    if (null === mapping) {
      mapping = {};
    } else if (safeMode && _hasOwnProperty.call(mapping, key)) {
  ...
  function readFlowCollection(nodeIndent) {
    ...
    while (position < length) {
      ...
      if (isMapping) {
        if (safeMode && _hasOwnProperty.call(_result, node1)) {
          throwError('duplication of a mapping key');
        }
        _result[node1] = node2;
      } else if (isPair) {
