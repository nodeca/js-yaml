...
function readLineBreak(state) {
  if (0x0A/* LF */ === state.character) {
    state.position += 1;
  } else if (0x0D/* CR */ === state.character) {
    if (0x0A/* LF */ === state.input.charCodeAt(state.position + 1)) {
      state.position += 2;
    } else {
      state.position += 1;
    }
...
function skipSeparationSpace(state, allowComments, checkIndent) {
  ...
  while (state.position < state.length) {
    while (is_WHITE_SPACE(state.character)) {
      state.character = state.input.charCodeAt(++state.position);
    }

    if (allowComments && CHAR_SHARP === state.character) {
      do { state.character = state.input.charCodeAt(++state.position); }
      while (state.position < state.length && !is_EOL(state.character));
    }

    if (is_EOL(state.character)) {
      readLineBreak(state);
      lineBreaks += 1;
      state.lineIndent = 0;

      while (0x20/* Space */ === state.character) {
        state.lineIndent += 1;
        state.character = state.input.charCodeAt(++state.position);
      }
...
function testDocumentSeparator(state) {
  ...
  if (state.position === state.lineStart &&
    ...
    if (_position >= state.length || is_WS_OR_EOL(_character)) {
      return true;
    }
  }
...
function readPlainScalar(state, nodeIndent, withinFlowCollection) {
  ...
  if (is_WS_OR_EOL(state.character) ||
      is_FLOW_INDICATOR(state.character) ||
  ...
  while (state.position < state.length) {
    if (CHAR_COLON === state.character) {
      ...
      if (is_WS_OR_EOL(preceding)) {
        break;
               ...
               withinFlowCollection && is_FLOW_INDICATOR(state.character)) {
      break;

    } else if (is_EOL(state.character)) {
      _line = state.line;
    ...
    if (!is_WHITE_SPACE(state.character)) {
...
function readDoubleQuotedScalar(state, nodeIndent) {
  ...
  while (state.position < state.length) {
    if (CHAR_DOUBLE_QUOTE === state.character) {
      ...
      if (is_EOL(state.character)) {
      ...
      } else if ((tmp = escapedHexBase(state.character)) > 0) {
        hexLength = escapedHexBase(state.character);
        hexResult = 0;
        ...
        for (hexIndex = 1; hexIndex <= hexLength; hexIndex += 1) {
          ...
          if ((tmp = fromHexCode(state.character)) >= 0) {
...
function readFlowCollection(state, nodeIndent) {
  ...
  if (state.character === 0x5B/* [ */) {
    terminator = 0x5D/* ] */;
    isMapping = false;
    _result = [];
  } else if (state.character === 0x7B/* { */) {
    terminator = 0x7D/* } */;
    isMapping = true;
    _result = {};
  } else {
...
function readBlockScalar(state, nodeIndent) {
  ...
  if (state.character === CHAR_VERTICAL_LINE) {
    folding = false;
  } else if (state.character === CHAR_GREATER_THAN) {
    folding = true;
  } else {
    return false;
  }
  ...
  while (state.position < state.length) {
    ...
    } else if ((tmp = fromDecimalCode(state.character)) >= 0) {
      if (tmp === 0) {
...
function readTagProperty(state) {
  ...
  if (isVerbatim) {
    ...
    while (state.position < state.length && !is_WS_OR_EOL(state.character)) {
