...
function loadAll(input, output, settings) {
  ...
  function readFlowCollection(nodeIndent) {
    ...
    while (position < length) {
      ...
      _line = line;
      composeNode(nodeIndent, CONTEXT_FLOW_IN, false);
      node1tag = tag;
      node1 = result;
  ...
  function readBlockSequence(nodeIndent) {
    var following, _tag = tag, _result = [], detected = false;
    ...
    while (position < length) {
      ...
      if (composeNode(nodeIndent, CONTEXT_BLOCK_IN, false)) {
        _result.push(result);
      } else {
        _result.push(null);
      }
  ...
  function readBlockMapping(nodeIndent) {
    ...
    while (position < length) {
      ...
      } else if (composeNode(nodeIndent, CONTEXT_FLOW_OUT, false)) {
        if (line === _line) {
      ...
      if (line === _line || lineIndent > nodeIndent) {
        if (composeNode(nodeIndent, CONTEXT_BLOCK_OUT, true)) {
          if (atExplicitKey) {
            keyNode = result;
          } else {
            valueNode = result;
  ...
  function composeNode(parentIndent, nodeContext, allowToSeek) {
    ...
    if (isIndented) {
      while (readTagProperty() || readAnchorProperty()) {
        if (skipSeparationSpace(-1)) {
          if (lineIndent > parentIndent) {
            isIndented = allowBlockCollections = true;
          } else if (lineIndent === parentIndent) {
            isIndented = false;
  ...
  function readDocument() {
    ...
    composeNode(lineIndent - 1, CONTEXT_BLOCK_OUT, false);
    output(result);
