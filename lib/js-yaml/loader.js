...
function readLineBreak(state) {
  ...
  if (0x0A/* LF */ === ch) {
    ...
    pos++;
    if (pos < len && 0x0A/* LF */ === input.charCodeAt(pos)) {
      pos++;
    }
  } else {
    throwError(state, 'a line break is expected');
...
function skipSeparationSpace(state, allowComments, checkIndent) {
  ...
  while (pos < len) {
    ...
    if (is_EOL(ch)) {
      ...
      ch = (pos < len) ? input.charCodeAt(pos) : 0;
      lineBreaks += 1;
      state.lineIndent = 0;
...
function testDocumentSeparator(state) {
  ...
  if (pos === state.lineStart &&
    ...
    if (pos >= len || is_WS_OR_EOL(input.charCodeAt(pos))) {
      return true;
    }
  }
...
function readPlainScalar(state, nodeIndent, withinFlowCollection) {
  ...
  if (0x3F/* ? */ === ch || 0x2D/* - */ === ch) {
    following = (state.position < len) ? input.charCodeAt(state.position+1) : 0;
