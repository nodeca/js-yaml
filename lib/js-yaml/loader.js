...
function loadAll(input, output, options) {
  ...
  function generateError(message) {
    return new YAMLException(
      message,
      new Mark(state.filename, input, state.position, state.line, (state.position - state.lineStart)));
  }
  ...
  function readLineBreak() {
    if (CHAR_LINE_FEED === state.character) {
      ...
      if (CHAR_LINE_FEED === input.charCodeAt(state.position + 1)) {
        state.position += 2;
      } else {
        state.position += 1;
      }
    ...
    state.character = input.charCodeAt(state.position);
  }
  ...
  function skipSeparationSpace(allowComments, checkIndent) {
    ...
    while (state.position < state.length) {
      ...
      if (allowComments && CHAR_SHARP === state.character) {
        do { state.character = input.charCodeAt(++state.position); }
        while (state.position < state.length &&
  ...
  function testDocumentSeparator() {
    ...
    if (state.position === state.lineStart &&
        ...
        input.charCodeAt(state.position + 1) === state.character &&
        input.charCodeAt(state.position + 2) === state.character) {
  ...
  function readPlainScalar(nodeIndent, withinFlowCollection) {
    ...
    while (state.position < state.length) {
      ...
      } else if ((state.position === state.lineStart && testDocumentSeparator()) ||
                 withinFlowCollection &&
                 (CHAR_COMMA                === state.character ||
                  CHAR_LEFT_SQUARE_BRACKET  === state.character ||
                  CHAR_RIGHT_SQUARE_BRACKET === state.character ||
      ...
      if (hasPendingContent) {
        ...
        writeFoldedLines(state.line - _line);
        captureStart = captureEnd = state.position;
        hasPendingContent = false;
      }
  ...
  function readSingleQuotedScalar(nodeIndent) {
    ...
    while (state.position < state.length) {
      if (CHAR_SINGLE_QUOTE === state.character) {
        captureSegment(captureStart, state.position, true);
        state.character = input.charCodeAt(++state.position);
  ...
  function readBlockScalar(nodeIndent) {
    ...
    while (state.position < state.length) {
      ...
      captureSegment(captureStart, state.position, false);
    }
  ...
  function readTagProperty() {
    ...
    if (isVerbatim) {
      ...
      if (state.position < state.length) {
        tagName = input.slice(_position, state.position);
        state.character = input.charCodeAt(++state.position);
      } else {
      ...
      while (state.position < state.length &&
        ...
        if (CHAR_EXCLAMATION === state.character) {
          if (!isNamed) {
            tagHandle = input.slice(_position - 1, state.position + 1);
  ...
  function readAnchorProperty() {
    ...
    state.anchor = input.slice(_position, state.position);
    return true;
  }
  ...
  function readAlias() {
    ...
    alias = input.slice(_position, state.position);
      ...
      if ('?' === state.tag) {
        if (state.resolve) {
          for (typeIndex = 0, typeQuantity = state.implicitTypes.length;
            ...
            _result = type.loader.resolver(state.result, false);
        ...
        if (null !== state.result && typeLoader.kind !== state.kind) {
          throwError('unacceptable node kind for !<' + state.tag + '> tag; it should be "' + typeLoader.kind + '", not "' + state.kind + '"');
        }

        if (typeLoader.resolver) {
          _result = typeLoader.resolver(state.result, true);

          if (NIL !== _result) {
            state.result = _result;
          } else {
            throwError('cannot resolve a node with !<' + state.tag + '> explicit tag');
          }
        }
      } else {
        throwWarning('unknown tag !<' + state.tag + '>');
      }
    }
  ...
  function readDocument() {
    ...
    while (state.position < state.length) {
      ...
      directiveName = input.slice(_position, state.position);
      directiveArgs = [];
      ...
      while (state.position < state.length) {
        ...
        directiveArgs.push(input.slice(_position, state.position));
      }
    ...
    if (0 === state.lineIndent &&
        CHAR_MINUS === state.character &&
        CHAR_MINUS === input.charCodeAt(state.position + 1) &&
        CHAR_MINUS === input.charCodeAt(state.position + 2)) {
      state.position += 3;
      state.character = input.charCodeAt(state.position);
      skipSeparationSpace(true, -1);
    ...
    composeNode(state.lineIndent - 1, CONTEXT_BLOCK_OUT, false, true);
    skipSeparationSpace(true, -1);

    if (state.validate && state.checkLineBreaks &&
        PATTERN_NON_ASCII_LINE_BREAKS.test(input.slice(documentStart, state.position))) {
      throwWarning('non-ASCII line breaks are interpreted as content');
    }

    output(state.result);

    if (state.position === state.lineStart && testDocumentSeparator()) {
  ...
  if (state.validate && PATTERN_NON_PRINTABLE.test(input)) {
