...
var NIL            = common.NIL;
var CORE_SCHEMA    = require('./schema/core');
...
function loadAll(input, output, settings) {
  ...
  directiveHandlers['YAML'] = function handleYamlDirective(name, args) {
    ...
    if (null === match) {
      throwError('ill-formed argument of a YAML directive');
    }
    ...
    if (1 !== major) {
      throwError('unacceptable YAML version of a document');
    }

    legacy = (minor < 2);

    if (1 !== minor && 2 !== minor) {
      throwWarning('unsupported YAML version of a document');
    }
  };
  ...
  directiveHandlers['TAG'] = function handleTagDirective(name, args) {
    ...
    if ('!' !== handle &&
        '!!' !== handle &&
        !(/^![a-z\-]+!$/i.test(handle))) {
      throwError('ill-formed tag handle (first argument) of a TAG directive');
    }

    if (!(/^(?:!|[^,\[\]\{\}])(?:%[0-9a-f]{2}|[a-z\-#;\/\?:@&=\+\$,_\.!~\*'\(\)\[\]])*$/i.test(prefix))) {
      throwError('ill-formed tag prefix (second argument) of a TAG directive');
    }
  ...
  function storeMappingPair(_result, keyTag, keyNode, valueNode) {
    ...
    } else if (validate && _hasOwnProperty.call(_result, keyNode)) {
      throwError('duplication of a mapping key');
    } else {
    ...
    while (position < length) {
      ...
      if (CHAR_LINE_FEED === character || CHAR_CARRIAGE_RETURN === character) {
        ...
        if (-1 !== expectedIndent && lineIndent < expectedIndent) {
          throwError('deficient indentation');
        }
      } else {
        break;
      }
  ...
  function performFlowFolding(expectedIndent, ensureIndented) {
    ...
    while (CHAR_LINE_FEED === character || CHAR_CARRIAGE_RETURN === character) {
      ...
      if (actualIndent < expectedIndent &&
        ...
        goodIndented = false;
        if (ensureIndented) {
          throwError('deficient indentation within a flow node');
        }
      }
  ...
  function readPlainScalar(nodeIndent, withinFlowNode) {
    ...
    while (position < length) {
      if (position === lineStart &&
          (CHAR_HYPHEN === character || CHAR_DOT === character)) {

        if (input.charCodeAt(position + 1) === character &&
            input.charCodeAt(position + 2) === character) {
          following = input.charCodeAt(position + 3);
  ...
  function readFlowCollection(nodeIndent) {
    var _tag = tag, _result, _line, following,
        terminator, isMapping, readNext = true,
        node1, node1tag, node2, pair, isPair, isExplicitPair;
    ...
    while (position < length) {
      ...
      if (character === terminator) {
        ...
        throwError('a missed comma between collection entries');
      }

      node1tag = node1 = node2 = null;
      ...
      _line = line;
      composeNode(nodeIndent, false, true);
      node1tag = tag;
      node1 = result;
      ...
      if (isMapping) {
        storeMappingPair(_result, node1tag, node1, node2);
      } else if (isPair) {
        _result.push(storeMappingPair(null, node1tag, node1, node2));
      } else {
        _result.push(node1);
      }
  ...
  // TODO: Cleanup.
  // FIXME: This reader should not increase the indent by more than +1.
  function readBlockSequence(nodeIndent) {
    var following, _nodeIndent, _tag = tag, _result = [], detected = false;
    ...
    while (position < length) {
      ...
      if (skipSeparationSpace(-1)) {
        if (lineIndent > nodeIndent) {
          ...
          throwError('unexpected content');
        }
      }

      if (composeNode(_nodeIndent, true, false)) {
        _result.push(result);
      } else {
        _result.push(null);
      }
  ...
  function readBlockMapping(nodeIndent) {
    ...
    while (position < length) {
      ...
      } else if (composeNode(nodeIndent, false, false)) {
        if (line === _line) {
  ...
  function readTagProperty() {
    ...
    if (isVerbatim) {
      ...
      if (_hasOwnProperty.call(tagMap, tagHandle)) {
        tagName = input.slice(_position, position);
        ...
        throwError('an undeclared tag handle');
      }
    }
    ...
    if (validate) {
      for (checkIndex = 0, checkLength = tagName.length;
        ...
        // FIXME: Check for two hexadecimal digits after the percent sign.
        if (!((CHAR_PERCENT === checkCharacter) ||
              ...
              (CHAR_SMALL_A <= checkCharacter && checkCharacter <= CHAR_SMALL_Z) ||
              (CHAR_CAPITAL_A <= checkCharacter && checkCharacter <= CHAR_CAPITAL_Z))) {
          throwError('tag name cannot contain such characters');
        }
      }
    }
  ...
  function composeNode(requiredIndent, allowBlockNodes, withinFlowNode) {
    var allowBlockCollections = allowBlockNodes,
        hasTag     = false,
        hasAnchor  = false,
        hasContent = false,
        type,
    ...
    if (readTagProperty()) {
      ...
      if (line === _line || lineIndent >= requiredIndent) {
        if (readAnchorProperty()) {
          hasAnchor = true;
        }
      }
      
    } else if (readAnchorProperty()) {
      hasAnchor = true;
    ...
    if (line === _line && (hasTag || hasAnchor)) {
    ...
    if (line === _line || lineIndent >= requiredIndent) {
      if (allowBlockCollections && (readBlockSequence(blockIndent) ||
          ...
          readFlowCollection(requiredIndent)) {
        hasContent = true;
        
      } else {
        if ((allowBlockNodes && readBlockScalar(requiredIndent)) ||
            readSingleQuotedScalar(requiredIndent) ||
            readDoubleQuotedScalar(requiredIndent) ||
            readAlias()) {
          hasContent = true;
          
        } else if (readPlainScalar(requiredIndent, withinFlowNode)) {
          hasContent = true;
    ...
    if ('!' !== tag) {
      if ('?' === tag) {
        for (typeIndex = 0, typeQuantity = implicitResolvers.length;
             typeIndex < typeQuantity;
             typeIndex += 1) {
          type = implicitResolvers[typeIndex];
      ...
      } else if (_hasOwnProperty.call(explicitResolvers, tag)) {
        type = explicitResolvers[tag];
        ...
        if (NIL !== _result) {
          ...
          throwError('cannot resolve a node with explicit tag !<' + tag + '>');
        }
      } else {
    ...
    return hasTag || hasAnchor || hasContent;
  }
  ...
  function readDocument() {
    ...
    composeNode(lineIndent, true, false);
    output(result);
    ...
    if (position < length) {
      throwError('end of the stream end or a document separator is expected');
    } else {
      return;
    }
  }
...
function safeLoadAll(input, output, settings) {
  ...
  common.extend(_settings, settings);

  _settings['validate'] = true;
  _settings['strict']   = true;
  _settings['schema']   = CORE_SCHEMA;

  loadAll(input, output, _settings);
}
