...
function skipSeparationSpace(state, allowComments, checkIndent) {
  ...
  while (pos < len) {
    ...
    if (allowComments && CHAR_SHARP === ch) {
...
function readPlainScalar(state, nodeIndent, withinFlowCollection) {
  ...
  if (is_WS_OR_EOL(ch) ||
      ...
      CHAR_ASTERISK             === ch ||
      CHAR_EXCLAMATION          === ch ||
      CHAR_VERTICAL_LINE        === ch ||
      CHAR_GREATER_THAN         === ch ||
      CHAR_SINGLE_QUOTE         === ch ||
      CHAR_DOUBLE_QUOTE         === ch ||
      CHAR_PERCENT              === ch ||
      CHAR_COMMERCIAL_AT        === ch ||
      CHAR_GRAVE_ACCENT         === ch) {
    return false;
  }

  if (CHAR_QUESTION === ch || CHAR_MINUS === ch) {
...
function readFlowCollection(state, nodeIndent) {
  ...
  while (state.position < len) {
    ...
    if ((isExplicitPair || state.line === _line) && CHAR_COLON === ch) {
      isPair = true;
      ch = input.charCodeAt(++state.position);
      skipSeparationSpace(state, true, nodeIndent);
      composeNode(state, nodeIndent, CONTEXT_FLOW_IN, false, true);
...
function readBlockScalar(state, nodeIndent) {
  ...
  if (ch === CHAR_VERTICAL_LINE) {
    folding = false;
  } else if (ch === CHAR_GREATER_THAN) {
    folding = true;
  } else {
    return false;
  }
  ...
  while (state.position < len) {
    ...
    if (CHAR_PLUS === ch || CHAR_MINUS === ch) {
      if (CHOMPING_CLIP === chomping) {
        chomping = (CHAR_PLUS === ch) ? CHOMPING_KEEP : CHOMPING_STRIP;
      } else {
...
function readTagProperty(state) {
  ...
  if (CHAR_LESS_THAN === ch) {
    isVerbatim = true;
    ch = input.charCodeAt(++state.position);

  } else if (CHAR_EXCLAMATION === ch) {
    isNamed = true;
    tagHandle = '!!';
    ch = input.charCodeAt(++state.position);
  ...
  if (isVerbatim) {
    do { ch = input.charCodeAt(++state.position); }
    while (state.position < len && CHAR_GREATER_THAN !== ch);
...
function readDocument(state, iterator) {
  ...
  while (state.position < len) {
    ...
    if (state.lineIndent > 0 || CHAR_PERCENT !== ch) {
      break;
    }
