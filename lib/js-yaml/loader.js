...
var NIL            = common.NIL;
var CORE_SCHEMA    = require('./schema/default');
...
function loadAll(input, output, settings) {
  ...
  if (undefined !== settings && null !== settings) {
    if (undefined !== settings.name && null !== settings.name) {
      name = settings.name;
    }
  }

  if (undefined !== settings && null !== settings) {
    if (undefined !== settings.validate && null !== settings.validate) {
      safeMode = settings.validate;
    }
  }
  ...
  function readBlockMapping(nodeIndent) {
    ...
    while (position < length) {
      ...
      if ((CHAR_QUESTION === character ||
        ...
        throwError('cannot read a block mapping entry');
